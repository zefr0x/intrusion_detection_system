...
pub fn uploaded_data_sizes_analyzer() {
	let mut guard = caches::TOTAL_DATA_SIZE_CACHE.0.lock().unwrap();
...
pub fn ports_activity_analyzer() {
	let mut guard = caches::PORTS_TOUCHED_CACHE.0.lock().unwrap();
...
pub fn dns_analyzer() {
	let mut guard = caches::DNS_CACHE.0.lock().unwrap();
...
pub fn syn_flood_analyzer() {
	let mut guard = caches::TCP_SYN_FLOOD_CACHE.0.lock().unwrap();
